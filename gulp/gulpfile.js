...
const changed = require('gulp-changed');//差分取得
...
const imageCompression = () =>{
  ...
  .pipe(
    imagemin([
      ...
      imagemin.svgo(),// svgを圧縮する必要がない時はコメントアウト
      imagemin.optipng(),
...
const browserSyncOption = {
  proxy: "localhost:10019",// ローカルにある「Site Domain」に合わせる
...
const watchFilesDev = () => {
  ...
  gulp.watch(srcPath.img, gulp.series(imageWebp, browserSyncReload));//ImgWebp化
  gulp.watch(srcPath.img, gulp.series(imageCompression, browserSyncReload));//Img圧縮
...
exports.dev = gulp.series(
  gulp.parallel(php, jsDev, cssSass, imageWebp, imageCompression),//初回起動時のみ実行
...
exports.default = gulp.series(
  gulp.parallel(php, jsProd, cssSass, imageWebp, imageCompression),//初回起動時のみ実行
